...
if(isset($_POST['submit'])){
    ...
    $query_phone = "SELECT * FROM user_details WHERE Phone_Number = '$Phone_Number' AND ID != $User_ID";
    $query_email = "SELECT * FROM user_details WHERE Email ='$Email' AND ID != $User_ID";
    $query_license = "SELECT * FROM diagnostic_center WHERE License_Number = $License_Number AND ID != $User_ID";
    ...
    if ($count > 0) {
        echo "<script type=\"text/javascript\">window.alert('Phone Number Already Exists!');window.location.href = '/chms/diagnostic-center/edit-profile.php';</script>";
    }

    else if ($count = mysqli_fetch_row($result_email) > 0) {
        echo "<script type=\"text/javascript\">window.alert('Email Address Already Exists!');window.location.href = '/chms/diagnostic-center/edit-profile.php';</script>";
    }
    else if ($count = mysqli_fetch_row($result_license) > 0) {
        echo "<script type=\"text/javascript\">window.alert('License Number Already Exists!');window.location.href = '/chms/diagnostic-center/edit-profile.php';</script>";
    }

    else{
        $sql_check = "SELECT * FROM diagnostic_center d, user_details u WHERE d.ID = u.ID AND u.ID = ". $User_ID ."";
        ...
        if(mysqli_num_rows($result_check) == 1){
            ...
            //modify diagnostic-center license fname lname dob gender experience position location language bio
            $sql_update_diagnostic_center = "UPDATE diagnostic_center
                                            SET License_Number = $License_Number, Name = '$Name'
                                            WHERE ID = $User_ID";

            if (mysqli_query($conn, $sql_update_user) && mysqli_query($conn, $sql_update_diagnostic_center)){
            ...
            //insert hospital
            $sql_insert_diagnostic_center = "INSERT INTO diagnostic_center(ID, License_Number, Name)
                                                VALUES ($User_ID, $License_Number, '$Name')";

            if (mysqli_query($conn, $sql_update_user) && mysqli_query($conn, $sql_insert_diagnostic_center)){
                echo "<script type=\"text/javascript\">window.alert('Profile Successfully Updated.');window.location.href = '/chms/common/profile.php';</script>";
            } 
            else{
                echo "Error: " . $sql . "" . mysqli_error($conn);
...
$sql = "SELECT d.Name, d.License_Number, u.Phone_Number, u.Email, u.Address
            FROM diagnostic_center d, user_details u
...
<form action="" method="POST" enctype="multipart/form-data">
    <table align="center">
        ...
        <tr>
            <td>
                <label for="Name">Diagnostic Center Name<font color="red">*</font>: </label>
            </td>
            <td>
        ...
        <tr>
            <td>
                <label for="Address">Diagnostic Center Location<font color="red">*</font>: </label>
            </td>
            <td>
        ...
        $sql_retrive = "SELECT u.Phone_Number
                        FROM user_details u WHERE u.ID = $User_ID";
...
<head>
    ...
    <title>Profile Edit - Diagnostic Center</title>
</head>
<body>
