...
if(isset($_POST['submit'])){
	...
	$sql_medicine_list = "SELECT m.ID, m.Name, m.Strength, p.Morning, p.M_After_Meal, p.Noon, p.N_After_Meal, p.Night, p.Nt_After_Meal
				...
				<td><div id="joy_congratulation">';
					if (mysqli_num_rows($result_medicine_list) > 0){
						while ($row = mysqli_fetch_assoc($result_medicine_list)){
							...
							echo '<form action="/chms/common/medicine-details.php" target="_blank" method="POST" style="margin: 0px; padding: 0px;">
									<input type="hidden" id="ID" name="ID" value="'.$row['ID'].'">
									<input type="submit" name="submit" value="'.$row['Name'].' '.$row['Strength'].'">
								</form>';						
...
<head>
	...
	<title>Prescriptions - Patient</title>
</head>
<body>
